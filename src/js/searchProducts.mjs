...
export default class SearchProducts {
  ...
  async init() {
    ...
    list.length > 0 ? this.renderList(list) : this.listElement.innerHTML = "<h2>No data found with the search criteria</h2>";

  }

  listSearchResults(dataSource) {
    return dataSource.filter(item => stringToDotNotation(item, this.searchValue).toLowerCase().includes(this.searchText.toLowerCase()))
  }
