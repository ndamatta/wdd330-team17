import { formDataToJSON, getLocalStorage, renderWithTemplate, setLocalStorage, updateCartBadge } from "./utils.mjs"
...
const packageItems = (items, formData, orderTotal, shipping, tax) => {
    const packedItems = items.map(item => {
        let newItem = { "id": item.Id, "name": item.Name, "price": item.ListPrice, "quantity": item.qty }
        return newItem
    });
...
export default class CheckoutProcess {
    ...
    calculateSummary() {
        this.itemTotal = Math.round(this.list.reduce((acumulator, item) => acumulator + (item.qty * item.ListPrice), 0) * 100) / 100
        this.calculateTax()
    ...
    calculateShipping() {
        if (this.list.length > 0) {
            const totalItems = this.list.reduce((acumulator, item) => acumulator + item.qty, 0)
    ...
    displayOrderTotals() {
        ...
        const template = renderSumary(this.itemTotal, this.shipping, this.tax, this.orderTotal)
    ...
    async generatePayload() {
        try {
            let payload = packageItems(this.list, this.formData, this.orderTotal, this.shipping, this.tax)
            const options = {
                method: "POST",
                headers: {
